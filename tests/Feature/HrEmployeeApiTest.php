...
class HrEmployeeApiTest extends TenantHrTestCase
{
    ...
    public function test_sales_employee_keeps_role_and_gains_smart_assistant_permission(): void
    {
        $headers = $this->hrHeaders();
        $sales = Role::query()->firstOrCreate(['slug' => 'sales'], ['name' => 'مبيعات']);
        $salesKeys = ['invoices.view', 'customers.view'];
        $salesIds = Permission::query()->whereIn('key', $salesKeys)->pluck('id')->all();
        $sales->permissions()->sync($salesIds);

        $assistantId = (int) Permission::query()->where('key', 'smart_assistant.access')->value('id');
        $this->assertGreaterThan(0, $assistantId);

        $email = 'sales.sa.'.uniqid().'@tenant.test';
        $payload = array_merge($this->employeePayload('EMP-SA-'.uniqid(), null), [
            'user_account' => [
                'email' => $email,
                'password' => 'SecretPass1',
                'password_confirmation' => 'SecretPass1',
                'role_id' => $sales->id,
                'permission_ids' => array_merge($salesIds, [$assistantId]),
            ],
        ]);

        $response = $this->postJson('/api/tenant/hr/employees', $payload, $headers)->assertCreated();
        $userId = (int) $response->json('data.user_id');
        $user = User::query()->findOrFail($userId);
        $keys = $user->roles()->with('permissions:id,key')->get()->pluck('permissions')->flatten()->pluck('key')->all();
        $this->assertContains('smart_assistant.access', $keys);
        $this->assertContains('invoices.view', $keys);
        $this->assertSame('hr-staff-'.$userId, $user->roles()->first()?->slug);
    }

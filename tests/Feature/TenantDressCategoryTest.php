...
class TenantDressCategoryTest extends TestCase
{
    ...
    public function test_can_create_parent_category(): void
    {
        ...
        // Parent create auto-seeds a default subcategory for operational dress/PO flows.
        $this->assertDatabaseHas('dress_categories', [
            'name' => 'عام',
            'parent_id' => DressCategory::query()->where('name', 'Bridal')->value('id'),
        ], 'tenant');
    }

    public function test_default_catalog_is_seeded_for_new_tenants_and_is_idempotent(): void
    {
        Artisan::call('db:seed', [
            '--database' => 'tenant',
            '--class' => \Database\Seeders\Tenant\DressCategorySeeder::class,
            '--force' => true,
        ]);
        Artisan::call('db:seed', [
            '--database' => 'tenant',
        ...
        $this->assertSame(3, DressCategory::query()->whereNull('parent_id')->count());
        $this->assertSame(11, DressCategory::query()->count());

        $dress = DressCategory::query()->where('name', 'فستان')->whereNull('parent_id')->firstOrFail();
        $wedding = DressCategory::query()->where('name', 'زفاف')->whereNull('parent_id')->firstOrFail();
        $engagement = DressCategory::query()->where('name', 'خطوبة')->whereNull('parent_id')->firstOrFail();

        $this->assertEqualsCanonicalizing(
            ['تركي', 'فرنسي', 'هاند ميد'],
            DressCategory::query()->where('parent_id', $dress->id)->pluck('name')->all(),
        );
        $this->assertEqualsCanonicalizing(
            ['تركي', 'فرنسي', 'هاند ميد'],
            DressCategory::query()->where('parent_id', $wedding->id)->pluck('name')->all(),
        );
        $this->assertEqualsCanonicalizing(
            ['كريمي', 'هاند ميد'],
            DressCategory::query()->where('parent_id', $engagement->id)->pluck('name')->all(),
        );
    }

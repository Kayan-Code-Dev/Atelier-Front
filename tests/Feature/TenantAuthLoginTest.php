...
class TenantAuthLoginTest extends TestCase
{
    ...
    public function test_tenant_can_login_with_google_id_token(): void
    {
        $tenant = $this->createTenant('atelier-google');
        $this->connectTenant($tenant);
        $this->createOwnerUser('owner@google.test', 'secret123');
        app(TenantUserDirectoryService::class)->register($tenant, 'owner@google.test');

        Config::set('services.google.client_id', 'test-google-client.apps.googleusercontent.com');

        $verifier = \Mockery::mock(\App\Services\Auth\GoogleIdTokenVerifier::class);
        $verifier->shouldReceive('verify')
            ->once()
            ->with('fake-google-id-token')
            ->andReturn([
                'email' => 'owner@google.test',
                'sub' => 'google-sub-1',
                'name' => 'Owner User',
                'picture' => null,
                'email_verified' => true,
            ]);
        $this->app->instance(\App\Services\Auth\GoogleIdTokenVerifier::class, $verifier);

        $response = $this->postJson('/api/tenant/login/google', [
            'id_token' => 'fake-google-id-token',
        ], ['Accept' => 'application/json']);

        $response->assertOk()
            ->assertJsonPath('success', true)
            ->assertJsonPath('data.tenant.slug', 'atelier-google')
            ->assertJsonPath('data.user.email', 'owner@google.test');

        $this->assertNotEmpty($response->json('data.token'));
    }

    public function test_google_auth_config_reports_enabled_state(): void
    {
        Config::set('services.google.client_id', 'client.apps.googleusercontent.com');

        $this->getJson('/api/tenant/auth/google')
            ->assertOk()
            ->assertJsonPath('data.enabled', true)
            ->assertJsonPath('data.client_id', 'client.apps.googleusercontent.com');
    }

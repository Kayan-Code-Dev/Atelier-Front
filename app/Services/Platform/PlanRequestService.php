...
class PlanRequestService
{
    public function __construct(
        ...
        private readonly PlanSignupOtpService $planSignupOtpService,
    ...
    public function store(array $data): array
    {
        ...
        $this->planSignupOtpService->assertVerifiedToken(
            $email,
            isset($data['email_verification_token']) ? (string) $data['email_verification_token'] : null,
        );

        $existing = PlanRequest::query()->where('email', $email)->first();
        if ($existing !== null) {
            $result = $this->handleExistingRequest($existing, $data, $plan, $isFreePlan);
            $this->planSignupOtpService->consumeToken((string) $data['email_verification_token']);
        ...
        $this->notifyPlatformAdmins($planRequest->fresh(['plan', 'paymentGateway']));
    ...
    private function handleExistingRequest(PlanRequest $existing, array $data, Plan $plan, bool $isFreePlan): array
    {
        ...
        if (in_array($existing->status, ['pending', 'rejected'], true)) {
            ...
            $this->notifyPlatformAdmins($existing->fresh(['plan', 'paymentGateway']));
    ...
    private function notifyPlatformAdmins(PlanRequest $planRequest): void
    {
        $error = $this->platformMailService->trySend(
            fn () => $this->platformMailService->notifyAdminsPlanRequest($planRequest),
        );

        if ($error !== null) {
            report(new \RuntimeException('Failed notifying admins about plan request: '.$error));
        }
    }
    ...
    public function publicStatus(PlanRequest $planRequest, string $email): array
    {
        ...
        return [
            ...
            'plan' => $planRequest->plan ? [
                ...
                'currency' => \App\Support\PlanCurrency::normalize($planRequest->plan->currency ?? 'EGP'),
                'currency_symbol' => \App\Support\PlanCurrency::symbol($planRequest->plan->currency ?? 'EGP'),
            ] : null,

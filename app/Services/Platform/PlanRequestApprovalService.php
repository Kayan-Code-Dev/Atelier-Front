...
class PlanRequestApprovalService
{
    ...
    public function approve(PlanRequest $planRequest, ?int $approvedBy = null): array
    {
        ...
        $this->platformMailService->trySend(
            fn () => $this->platformMailService->sendPlanRequestApprovedCredentials(
                (string) $planRequest->email,
                (string) $planRequest->name,
                $plainPassword,
                $loginHostname,
                (string) ($plan->name ?? 'باقتك'),
            ),
        );

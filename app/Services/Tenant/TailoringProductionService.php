...
class TailoringProductionService
{
    ...
    public function update(Invoice $invoice, array $data, ?int $actorId = null): Invoice
    {
        ...
        if (isset($data['employee_id']) && ! isset($data['assigned_tailor_id'])) {
            $employee = HrEmployee::query()->find((int) $data['employee_id']);
            if ($employee?->user_id) {
                $invoiceData['assigned_tailor_id'] = (int) $employee->user_id;
            }
        }

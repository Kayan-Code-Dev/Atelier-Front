...
class InvoiceService
{
    public function __construct(
        private readonly \App\Support\PlanLimitGuard $planLimitGuard,
        private readonly \App\Support\Tenant\AuthorizedBranchAccess $branchAccess,
        private readonly EmployeeCommissionService $commissionService,
    ) {}

    public function paginate(array $filters, int $perPage = 15): LengthAwarePaginator
    {
        $query = Invoice::query()
            ->with(['customer', 'branch', 'employee', 'items.dress.category', 'items.dress.subcategory'])
            ->latest('id');

        $this->branchAccess->constrain($query);
    ...
    {
        $data = $this->commissionService->attachEmployeeToInvoiceData($data, $actorId);
        $data = $this->branchAccess->applyInvoiceActorDefaults($data, $actorId);
        ...
        $this->planLimitGuard->assertCanCreateInvoice($invoiceType);
        ...
        $invoice = DB::connection('tenant')->transaction(function () use ($data, $items, $summary, $status, $actorId, $invoiceType): Invoice {
            ...
            $assignedTailorId = $data['assigned_tailor_id'] ?? null;
            if ($employeeId) {
                $employee = HrEmployee::query()->find($employeeId);
                if ($employee?->user_id && ! $assignedTailorId && $invoiceType === Invoice::TYPE_TAILORING) {
                    $assignedTailorId = (int) $employee->user_id;
                }
            }
        ...
        $invoice = $invoice->refresh()->load(['customer', 'branch', 'employee', 'items.dress.category', 'items.dress.subcategory', 'payments']);
        $this->commissionService->accrueForInvoice($invoice);
        app(OperationalNotificationService::class)->invoiceCreated($invoice);
        $this->notifyWhatsAppIfNewlyConfirmed($invoice, null);
    ...
    {
        $query = Invoice::query()
            ->with(['customer', 'branch', 'employee', 'items.dress.category', 'items.dress.subcategory', 'payments']);
        $this->branchAccess->constrain($query);
    ...
    public function update(Invoice $invoice, array $data, ?int $actorId = null): Invoice
    {
        ...
        $updatedInvoice = DB::connection('tenant')->transaction(function () use ($invoice, $data, $newItems, $summary, $newStatus, $newType, $actorId): Invoice {
            ...
            $invoice->fill([
                ...
                'employee_id' => array_key_exists('employee_id', $data) ? ($data['employee_id'] ?: null) : $invoice->employee_id,
                'type' => $newType,
                'status' => $newStatus,
                'subtotal' => $summary['subtotal'],
                'discount' => $summary['discount'],
        ...
        $fresh = $updatedInvoice->refresh()->load(['customer', 'branch', 'employee', 'items.dress.category', 'items.dress.subcategory', 'payments']);
        $this->commissionService->accrueForInvoice($fresh);
        $this->notifyWhatsAppIfNewlyConfirmed($fresh, $previousStatus);
    ...
    {
        $this->commissionService->voidForInvoice($invoice);
        $invoice->delete();
    }
    ...
    private function notifyWhatsAppIfNewlyConfirmed(Invoice $invoice, ?string $previousStatus): void
    {
        $confirmedLike = [
            Invoice::STATUS_CONFIRMED,
            Invoice::STATUS_PAID,
        ...
        if (! in_array((string) $invoice->status, $confirmedLike, true)) {
            return;
        }
        $already = array_merge($confirmedLike, [Invoice::STATUS_DELIVERED, Invoice::STATUS_RETURNED]);
        if ($previousStatus !== null && in_array($previousStatus, $already, true)) {
            return;
        }
        ...
        try {
            ...
            app(TenantWhatsAppInvoiceNotifier::class)->notifyConfirmed($tenant, $invoice);
        } catch (Throwable $e) {
            Log::warning('whatsapp.invoice_panel_notify_failed', [

...
class BranchService
{
    ...
    public function paginate(array $filters, int $perPage = 15): LengthAwarePaginator
    {
        ...
        app(\App\Support\Tenant\AuthorizedBranchAccess::class)->constrain($query, 'id');
    ...
    {
        $this->planLimitGuard->assertCanCreateBranch();
    ...
    private function payload(array $data, ?Branch $branch = null): array
    {
        ...
        return [
            ...
            'image' => array_key_exists('image', $data) ? $this->normalizeImageValue($data['image']) : $branch?->image,
            'logo' => array_key_exists('logo', $data) ? $this->normalizeImageValue($data['logo']) : $branch?->logo,
            'cover' => array_key_exists('cover', $data) ? $this->normalizeImageValue($data['cover']) : $branch?->cover,
    ...
    private function normalizeImageValue(mixed $value): ?string
    {
        ...
        // Keep data URLs and remote/local paths; reject absurdly large payloads (~3.5MB chars).
        if (strlen($normalized) > 3_500_000) {
            throw new \RuntimeException('حجم صورة الفرع كبير جداً. استخدم صورة أصغر.');
        }

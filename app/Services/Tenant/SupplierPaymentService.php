...
class SupplierPaymentService
{
    ...
    public function addPayment(Supplier $supplier, array $data, ?int $actorId = null): SupplierPayment
    {
        ...
        $payment = DB::connection('tenant')->transaction(function () use ($supplier, $purchaseOrder, $data, $actorId): SupplierPayment {
            ...
            if (empty($payment->cashbox_id)) {
                $defaultCashbox = Cashbox::query()
                    ->where('is_active', true)
                    ->when(
                        $purchaseOrder?->branch_id,
                        fn ($q) => $q->where('branch_id', $purchaseOrder->branch_id),
                    )
                    ->orderBy('id')
                    ->first()
                    ?? Cashbox::query()->where('is_active', true)->orderBy('id')->first();
                if ($defaultCashbox) {
                    $payment->cashbox_id = $defaultCashbox->id;
                    $payment->save();
                }
            }

            $this->cashMovementService->recordSupplierPayment($payment->refresh(), $actorId);

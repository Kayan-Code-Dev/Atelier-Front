...
class HrEmployeeAccountService
{
    ...
    public function createForEmployee(HrEmployee $employee, array $account): User
    {
        ...
        $this->planLimitGuard->assertCanCreateStaffUser();
    ...
    private function syncUserAccess(User $user, array $account): void
    {
        ...
        if ($roleId) {
            ...
            $rolePermissionIds = $role->permissions()->pluck('id')->map(static fn ($id) => (int) $id)->all();
            $merged = array_values(array_unique(array_merge($rolePermissionIds, $permissionIds)));
            sort($merged);
            $roleSorted = $rolePermissionIds;
            sort($roleSorted);
            if ($merged === $roleSorted) {
                $user->roles()->sync([$role->id]);

                return;
            }

            // Extra boxes vs the shared role (e.g. smart_assistant on a sales employee).
            $this->assignPersonalRole($user, $merged);
        ...
        $this->assignPersonalRole($user, $permissionIds);
    }

    /**
     * @param list<int> $permissionIds
     */
    private function assignPersonalRole(User $user, array $permissionIds): void
    {
        $validCount = Permission::query()->whereIn('id', $permissionIds)->count();
        if ($validCount !== count($permissionIds)) {
            throw ValidationException::withMessages([
    ...
    public function listPermissions(): array
    {
        return Permission::query()
            ...
            ->map(static function (Permission $permission): array {
                ...
                if (str_starts_with($key, 'smart_assistant.')) {
                    $group = 'المساعد الذكي';
                }
                ...
                return [
                    'id' => $permission->id,
                    'key' => $key,
                    'name' => \App\Support\PermissionLabels::label($key, (string) $permission->name),

...
class CashboxService
{
    ...
    {
        $query = Cashbox::query()
            ->with(['branch', 'account:id,code,name'])
            ...
            ->withSum(['movements as total_out' => fn (Builder $builder) => $builder
                ...
                ->where('is_reversed', false)], 'amount')
            ->withMax(['movements as last_movement_at' => fn (Builder $builder) => $builder->where('is_reversed', false)], 'movement_date')
            ->latest('id');
    ...
    {
        return Cashbox::query()->with(['branch', 'account:id,code,name'])->findOrFail($cashboxId);
    }
    ...
    public function update(Cashbox $cashbox, array $data): Cashbox
    {
        ...
        $hasMovements = CashMovement::query()->where('cashbox_id', $cashbox->id)->exists();
        $initialBalance = $hasMovements
            ? (float) $cashbox->initial_balance
            : round((float) ($data['initial_balance'] ?? $cashbox->initial_balance ?? 0), 2);

        $cashbox->fill([
            'name' => $data['name'],
            'kind' => $data['kind'] ?? $cashbox->kind ?? 'cash',
            'branch_id' => $data['branch_id'] ?? null,
            'account_id' => $data['account_id'] ?? $cashbox->account_id,
            'initial_balance' => $initialBalance,
            'description' => $data['description'] ?? null,
            'is_active' => (bool) ($data['is_active'] ?? true),
        ]);

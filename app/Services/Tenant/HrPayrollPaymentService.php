...
class HrPayrollPaymentService
{
     ...
     */
    public function historyForMonth(string $month, ?int $branchId = null): array
    {
        $period = Carbon::parse($month.'-01');

        $query = HrPayrollPayment::query()
        ...
        return $query->get()->map(function (HrPayrollPayment $payment): array {
            return [
                'id' => $payment->id,
                'employee_id' => $payment->employee_id,
                'employee_name' => $payment->employee?->full_name,
                'employee_code' => $payment->employee?->employee_code,
                'month' => $payment->payroll_month?->format('Y-m'),

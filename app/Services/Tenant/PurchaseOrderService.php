...
class PurchaseOrderService
{
    public function __construct(
        ...
        private readonly DressCategoryService $dressCategoryService,
    ...
    public function create(array $data, ?int $actorId = null): PurchaseOrder
    {
        ...
        $depositAmount = $this->money((float) ($data['deposit_amount'] ?? 0));
        if ($depositAmount > $summary['total']) {
            $depositAmount = $summary['total'];
        }

        /** @var PurchaseOrder $purchaseOrder */
        $purchaseOrder = DB::connection('tenant')->transaction(function () use ($data, $items, $summary, $actorId, $depositAmount): PurchaseOrder {
            ...
            if ($depositAmount > 0) {
                $payment = $this->recordDepositPayment(
            ...
            return $this->syncFinancials($purchaseOrder->refresh(), (string) $purchaseOrder->status);
        });
    ...
    public function update(PurchaseOrder $purchaseOrder, array $data, ?int $actorId = null): PurchaseOrder
    {
        ...
        $updated = DB::connection('tenant')->transaction(function () use ($purchaseOrder, $data, $items, $summary, $actorId): PurchaseOrder {
            $previousDeposit = $this->money((float) ($purchaseOrder->deposit_amount ?? 0));
            $nextDeposit = array_key_exists('deposit_amount', $data)
                ? $this->money((float) $data['deposit_amount'])
                : $previousDeposit;
            if ($nextDeposit > $summary['total']) {
                $nextDeposit = $summary['total'];
            }
            ...
            $depositDelta = $this->money($nextDeposit - $previousDeposit);
            if ($depositDelta > 0) {
                $payment = $this->recordDepositPayment(
                    purchaseOrder: $purchaseOrder,
                    amount: $depositDelta,
    ...
    public function receive(PurchaseOrder $purchaseOrder, ?int $actorId = null): PurchaseOrder
    {
        ...
        $purchaseOrder = DB::connection('tenant')->transaction(function () use ($purchaseOrder, $actorId): PurchaseOrder {
            // 1. Mark inventory as received — keep payment status intact
            ...
            foreach ($purchaseOrder->items as $index => $item) {
                ...
                $categoryId = $item->dress_category_id ?? $purchaseOrder->category_id;
                $subcategoryId = $item->dress_subcategory_id ?? $purchaseOrder->subcategory_id;

                // Align with StoreDressRequest: if a parent category is set but no subcategory,
                // attach/create the default child so inventory stays consistent with manual dress create.
                if ($categoryId && ! $subcategoryId) {
                    $parent = DressCategory::query()->find((int) $categoryId);
                    if ($parent !== null && $parent->parent_id === null) {
                        $subcategoryId = $this->dressCategoryService->ensureDefaultChild($parent)->id;
                    }
                }
                ...
                if ($subcategoryId && $subcategoryName === '') {
                    $subcategoryName = DressCategory::query()->find((int) $subcategoryId)?->name ?? '';
                }
                ...
                $dress = Dress::query()->create([
                    ...
                    'dress_category_id' => $categoryId,
                    'dress_subcategory_id' => $subcategoryId,
            ...
            // 3. Journal: Debit Inventory (1300) / Credit Suppliers AP (2000)
            $inventoryAccount = $this->findOrCreateAccount('1300', 'المخزون', 'asset');
    ...
    {
        // Payments table is the single source of truth (deposit is recorded as a payment).
        $paidAmount = $this->money((float) SupplierPayment::query()
            ->where('purchase_order_id', $purchaseOrder->id)
            ->sum('amount'));

        // Legacy fallback: old rows stored deposit without a payment entry.
        $depositAmount = $this->money((float) ($purchaseOrder->deposit_amount ?? 0));
        ...
        $preferred = $preferredStatus ?? (string) $purchaseOrder->status;
        // Normalize invalid legacy "received" status into payment-aware status
        if ($preferred === 'received') {
            $preferred = $purchaseOrder->received_at
    ...
    private function recordDepositPayment(
        PurchaseOrder $purchaseOrder,
        float $amount,
        ?string $method,
        ?int $cashboxId,
        ...
        $resolvedCashboxId = $cashboxId;
        if ($resolvedCashboxId === null || $resolvedCashboxId <= 0) {
            $resolvedCashboxId = $this->resolveDefaultCashboxId(
                isset($purchaseOrder->branch_id) ? (int) $purchaseOrder->branch_id : null
            );
        }
        ...
        return SupplierPayment::query()->create([
            ...
            'amount' => $amount,
            'method' => $method ?: 'cash',
            'reference' => 'DEP-'.$purchaseOrder->purchase_order_number,
            'paid_at' => $purchaseOrder->order_date ?? now(),
            'notes' => 'عربون طلبية شراء',
            'created_by' => $actorId,
        ]);
    }

    private function resolveDefaultCashboxId(?int $branchId): ?int
    {
        $query = Cashbox::query()->where('is_active', true)->orderBy('id');
        if ($branchId !== null && $branchId > 0) {
            $branchCashbox = (clone $query)->where('branch_id', $branchId)->first();
            if ($branchCashbox instanceof Cashbox) {
                return (int) $branchCashbox->id;
            }
        }
    ...
    private function calculateSummary(array $items, float $discount, float $tax): array
    {
        ...
        // Guard against clients that send tax ≈ subtotal and silently double the order total.
        if ($normalizedTax > 0 && $subtotal > 0 && $normalizedTax >= $subtotal) {
            $normalizedTax = 0.0;
        }

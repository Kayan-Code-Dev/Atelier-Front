...
class SupplierService
{
    ...
    public function withSummary(Supplier $supplier): Supplier
    {
        ...
        $supplier->setAttribute('total_purchases', $this->money($summary['total_purchase_orders']));
        $supplier->setAttribute('total_paid', $this->money($summary['total_paid']));
        $supplier->setAttribute('total_payment', $this->money($summary['total_paid']));
        $supplier->setAttribute('total_remaining', $this->money($summary['total_remaining']));
        $supplier->setAttribute('remaining', $this->money($summary['total_remaining']));
        $supplier->setAttribute('orders_count', (int) $summary['orders_count']);
        $supplier->setAttribute('purchases_count', (int) $summary['orders_count']);
    ...
    /**
     * @return array{total_purchase_orders:float,total_paid:float,total_remaining:float,unlinked_payments:float,orders_count:int}
     */
    private function summary(Supplier $supplier): array
    {
        ...
        $totalPurchaseOrders = $this->money((float) (clone $purchaseOrderQuery)->sum('total'));
        $totalRemaining = $this->money((float) (clone $purchaseOrderQuery)->sum('remaining_amount'));
        $ordersCount = (int) (clone $purchaseOrderQuery)->count();

...
use App\Http\Requests\Platform\PlanRequest\SendPlanSignupOtpRequest;
use App\Http\Requests\Platform\PlanRequest\StorePlanRequestRequest;
use App\Http\Requests\Platform\PlanRequest\VerifyPlanSignupOtpRequest;
...
use App\Services\Platform\PlanSignupOtpService;
...
class PlanRequestController extends Controller
{
    public function __construct(
        private readonly PlanRequestService $planRequestService,
    ...
    public function sendOtp(SendPlanSignupOtpRequest $request): JsonResponse
    {
        try {
            $result = $this->planSignupOtpService->send(
            ...
            return ApiResponse::error('تعذر إرسال رمز التحقق حالياً. حاول مرة أخرى.', 500);
        }

        return ApiResponse::success($result, (string) ($result['message'] ?? 'تم إرسال رمز التحقق'));
    }

    public function verifyOtp(VerifyPlanSignupOtpRequest $request): JsonResponse
    {
        try {
            $result = $this->planSignupOtpService->verify(
                (string) $request->validated('email'),
        ...
        return ApiResponse::success($result, (string) ($result['message'] ?? 'تم التحقق بنجاح'));
    }

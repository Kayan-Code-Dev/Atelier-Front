...
use App\Http\Requests\Tenant\Dress\DressRentalReportRequest;
...
use App\Services\Tenant\DressRentalReportService;
...
class DressController extends Controller
{
    public function __construct(
        ...
        private readonly DressRentalReportService $dressRentalReportService,
    ) {}
    ...
    public function rentalReport(DressRentalReportRequest $request, int $dress): JsonResponse
    {
        $dressModel = $this->dressService->findOrFail($dress);
        $report = $this->dressRentalReportService->build(
            dress: $dressModel,
            filters: $request->filters(),
            actor: $request->user(),
        );

        return ApiResponse::success($report, 'Dress rental report');
    }
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        return CsvExporter::download(
            ...
            headers: ['المعرّف', 'الكود', 'الاسم', 'الحالة', 'الفرع', 'التصنيف', 'التصنيف الفرعي', 'نوع الكيان', 'رقم الكيان', 'سعر الإيجار', 'سعر البيع', 'تاريخ الإنشاء'],
            rows: $rows,
        );
    }
}

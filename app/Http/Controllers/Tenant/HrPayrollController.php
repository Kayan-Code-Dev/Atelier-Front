...
class HrPayrollController extends Controller
{
    ...
    public function payslip(Request $request, int $employee): JsonResponse
    {
        ...
        $periodType = strtolower((string) ($request->query('period') ?: 'month'));
        if ($periodType === 'year') {
            $year = (int) ($request->query('year') ?: now()->year);

            return ApiResponse::success(
                $this->payrollService->yearlyPayslipForEmployee($employeeModel, $year)
            );
        }
    ...
    {
        $months = max(1, min(24, $request->integer('months', 24)));
    ...
    public function history(Request $request): JsonResponse
    {
        ...
        return ApiResponse::success($this->paymentService->historyForMonth($month, $branchId));
    }
}

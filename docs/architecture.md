...
## AI Agent Platform (architecture freeze)

DressnMore includes a planned independent bounded context: **AI Agent Platform** (Digital Employee OS). Channels (WhatsApp first) are adapters only; tenant isolation for channel traffic uses a Channel Binding Registry, not `X-Tenant` headers from customers.

Architecture pack (no code / DB / API / UI in the freeze):

- [`ai-agent-platform/README.md`](./ai-agent-platform/README.md)
- [`ai-agent-platform/ENTERPRISE-ARCHITECTURE.md`](./ai-agent-platform/ENTERPRISE-ARCHITECTURE.md)
